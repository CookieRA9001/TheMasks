...
namespace TheMasks.Items.RedMask{
    public class RedMaskSpawner : ModItem{
        public override void SetStaticDefaults(){
            DisplayName.SetDefault("Red Mask Summon");
            Tooltip.SetDefault("Summon The Red Mask");
        }
        ...
        public override bool CanUseItem(Player player){
            return !NPC.AnyNPCs(mod.NPCType("RedMaskShell"));
        }
        public override bool UseItem(Player player){
            Main.PlaySound(SoundID.Roar, player.position);
            if(Main.netMode != 1){
                NPC.SpawnOnPlayer(player.whoAmI, mod.NPCType("RedMaskShell"));
            }
            return true;
        }
