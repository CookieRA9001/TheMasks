...
namespace TheMasks.NPCs.Bosses.RedMask{
    [AutoloadBossHead]
    public class RedMaskShell : ModNPC {
        //animation
        ...
        int nextmode = 0;
        int att_count = 0;
        ModNPC eyeType = ModContent.GetModNPC(ModContent.NPCType<NPCs.Bosses.RedMask.RedMaskEyes>());
        ...
        public override void SetStaticDefaults(){
            DisplayName.SetDefault("The Red Mask");
        ...
        public override void ScaleExpertStats(int numPlayers, float bossLifeScale){
            npc.lifeMax = (int)(npc.lifeMax*power_scale) + numPlayers*2000;
            power_scale = 1.5f;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor){
            //draw back
            Texture2D MaskTexture = mod.GetTexture("NPCs/Bosses/RedMask/RedMask");
            ...
            //draw eyes
            Texture2D EyeTexture = mod.GetTexture("NPCs/Bosses/RedMask/RedMaskEyes");
        ...
        public override void PostDraw(SpriteBatch spriteBatch, Color drawColor){
            //draw gem
            Texture2D MaskTexture = mod.GetTexture("NPCs/Bosses/RedMask/RedGem");
        ...
        public override void AI(){
            //looks
            Lighting.AddLight(npc.position, 2f, 0.4f, 0.4f);
            //buff resistence
            if(npc.HasBuff(24)) {npc.DelBuff(npc.FindBuffIndex(24)); npc.life+=(int)(20*power_scale);}
            //player targeting
            ...
            switch(mode){
                case 0:{ // curcling
                    ...
                    if(PtoT!=250){
                        targX = pX + (targX-pX)*(250/PtoT);
                        targY = pY + (targY-pY)*(250/PtoT);
                    }
                ...
                case 2:{ // fire
                    if(nextmode==0){
                        ...
                        if((att_count%10)==0){
                            ...
                            shootPos.Y -= 24;
                            float ame = (15f-(npc.ai[0]*0.1f)) / power_scale;
                            Vector2 shootVel = player.Center - shootPos + new Vector2(Main.rand.NextFloat(-ame,ame),Main.rand.NextFloat(-ame,ame));
                            shootVel.Normalize();
                            shootVel *= 12f;
                            Projectile.NewProjectile(shootPos,shootVel,mod.ProjectileType("BadRedFire"),(int)(30*power_scale),1f);
                        }
                        if((att_count%50)==0){
                            Vector2 shootPos = npc.Center;
                            shootPos.Y -= 24;
                            for(int i = 1; i<5; i++){
                                float ame = (20f-(npc.ai[0]*0.1f))*i / power_scale;
                                Vector2 shootVel = player.Center - shootPos + new Vector2(Main.rand.NextFloat(-ame,ame),Main.rand.NextFloat(-ame,ame));
                                shootVel.Normalize();
                                shootVel *= 12f;
                                Projectile.NewProjectile(shootPos,shootVel,mod.ProjectileType("BadRedFire"),(int)(30*power_scale),1f);
            ...
            switch(animation){
                case 0:{
                    ...
                    npc.damage = (int)(40*power_scale);
                break;}
                case 1: case 3:{
                    npc.defense = (int)(12*power_scale);
                    npc.damage = (int)(44*power_scale);
                break;}
                case 2:{
                    npc.defense = (int)(8*power_scale);
        ...
        public override void BossLoot(ref string name, ref int potionType){
            //MyBG.life = 0;
            if(!Main.expertMode){
                Item.NewItem((int)npc.position.X,(int)npc.position.Y,npc.width,npc.height,mod.ItemType("RedEssence"),Main.rand.Next(10,15));
                ...
                Item.NewItem((int)npc.position.X,(int)npc.position.Y,npc.width,npc.height,mod.ItemType("RedEssence"),Main.rand.Next(12,16));
